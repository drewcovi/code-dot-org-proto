...
const quests = [
  ...
  },
  {
    ...
    finalProject:
      'Build a campaign site, chatbot, or data visualization for social impact.',
  ...
  },
  {
    ...
    icon: '📱',
    objective:
      'Demystify the algorithms behind notifications, feeds, and recommendations.',
    finalProject:
      'Prototype a smart feature like a to-do reminder or custom alert system.',
...
const ctPillars = [
  {
    ...
    scenario:
      'Students join DJ Signal in building the ultimate remix set by breaking down musi' +
      'cal problems into beats, loops, and layers.',
  },
  {
    title: 'Unit 2: Programming Basics and Data',
    scenario:
      'Students code the backend of a futuristic music café where beat recipes (songs) ' +
      'are built using variables, lists, and objects.',
  },
  {
    title: 'Unit 3: Control Structures',
    scenario:
      'Compete in a global electronic music festival where your performance depends on ' +
      'coding sound sequences with loops, conditionals, and functions.',
  },
  {
    title: 'Unit 4: Testing and Debugging',
    scenario:
      "You’re in a high-stakes studio session. A superstar's track has bugs in the auto" +
      'mation scripts—can you save the session before the drop?',
  },
  {
    title: 'Capstone: Code DJ Showcase',
    scenario:
      'Create and perform your own Python-powered musical project—a visualizer, beat ge' +
...
export default function QuestSelector() {
  ...
  return (
    <>
    <title>Quest selector</title>
      <Header
        title="Pick a quest"
        avatar={true}
      >
<p className="text-muted-foreground">
          How would you like to learn Python Programming? Select an interests while building computational
          thinking skills!
        </p>
      </Header>
      <main className="grid grid-cols-3 overflow-y-scroll">
        <div className="flex overflow-y-scroll flex-col p-6 max-w-screen-lg mx-auto pb-40">
        ...
        {selectedQuest && (
          <div className="flex flex-col col-span-2 p-6 overflow-y-scroll pb-40">
            <h2 className="text-2xl font-bold text-center mb-2">
              {selectedQuest.title} Quest Map
            </h2>
      ...
      <Footer>
        <button
          className="px-8 py-4 rounded-xl text-white bg-gray-500"
