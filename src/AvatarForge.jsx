...
const catalog = {
  "Skin Tone": [
    { name: "Light", group: "skin-light", prompt: "asian (light) skintone", color: "#FCD7B8" },
    { name: "Medium", group: "skin-medium", prompt: "caucasian skintone", color: "#E0B394" },
    { name: "Olive", group: "skin-olive", prompt: "hispanic (olive) skintone", color: "#C68642" },
  ...
  "Hair & Headwear": [
    ...
    { name: "Baseball Cap", group: "hair-baseball-cap", prompt: "blue baseball hat" },
    { name: "Hijab", group: "hair-headwear-hijab", prompt: "hijab" },
    { name: "Shag", group: "hair-shag", prompt: "shag hair" },
    { name: "Short Dreads", group: "hair-short-dreads", prompt: "short dreadlocs" },
...
function buildPrompt(selections) {
  const parts = categories.map((c) => catalog[c].find((o) => o.group === selections[c].group).prompt).filter(Boolean);
  return `cartoon avatar, ${parts.join(", ")}, flat vector style, isolated single person portrait, avatar style, circle cropped, full-bleed`;
}
...
export default function AvatarForge() {
  ...
  // const [codemate, setCodemate] = useState(()=>{
  //   let init = '';
  //   return init;
  // });
  ...
  const handleGenerate = async () => {
    ...
    try {
      const resp = await fetch("https://api.openai.com/v1/images/generations", {
        ...
        body: JSON.stringify({ model: "dall-e-3", prompt: buildPrompt(selections), n: 1, size: "1024x1024" })
      });
  ...
  const renderNav = () => (
    <nav className="space-y-2 text-sm">
      {categories.map((cat) => (
        <button
          key={cat}
          className={`block w-full text-left px-3 py-2 rounded-lg ${currentCat === cat ? "bg-indigo-100 font-medium" : "hover:bg-gray-100"}`}
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-4 w-full max-w-4xl">
       {catalog[currentCat].map((opt) => (
        <button
          ...
          className={`flex flex-col items-center justify-center border rounded-lg p-2 text-xs space-y-1 ${selections[currentCat].group === opt.group ? "border-indigo-600 bg-indigo-50" : "hover:bg-gray-50"}`}
        >
          <span>{opt.name}</span>
        </button>
      ))}
  ...
  const isVisible = (group) => Object.values(selections).some((sel) => sel.group === group);
      ...
      <main className="flex flex-1 overflow-y-scroll">
        ...
        <section className="flex-1 pb-40 flex flex-col items-center justify-top p-6 space-y-6 overflow-y-auto">
          ...
          <div className="relative w-64 h-64 select-none rounded-full" style={avatarStyles}>
            ...
            {/* AI IMAGE OVERLAY */}
            {aiUrl && <img src={aiUrl} alt="AI Avatar" className="absolute inset-0 w-full h-full object-cover" />}
            {/* PRONOUN TAG */}
        ...
        <aside className="bg-gray-50 w-64 rounded-xl my-10 border-l border-gray-200 p-4 overflow-y-auto">
          ...
          <ul className="space-y-2 text-sm">
            {categories.map((cat) => (
              <li key={cat}>{cat}: {selections[cat].name}</li>
            ))}
          </ul>
          <div className="py-5">
          <button onClick={handleRandomize} className="flex-none flex px-4 py-2 bg-gray-200 rounded-xl shadow text-sm">Randomize</button>
          </div>
        </aside>
      </main>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-2xl shadow-xl w-80">
            ...
            <p className="text-sm text-gray-600 mb-3">Enter your secret key to enable AI image generation.</p>
            <input
              type="password"
              value={apiKey}
